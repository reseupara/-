				...
				<div class="row">
					...
					<div class="col-md-12">
						...
						<div class="box box-primary">
							...
							<form id='registerForm' role="form" action="/admin/product/modify" method="post" enctype="multipart/form-data">
								<div class="box-body">
									<div class="form-group">
										...
										<select class="form-control" id="mainCategory" name="cg_code_up" style="width:30%; margin-right:10px; display: inline-block;" >
											...
											<c:forEach items="${categoryList }" var="cate">
												<option value='<c:out value="${cate.cg_code }" />' <c:out value="${cate.cg_code == productVO.cg_code_up ? 'selected' : ''}" />>
									...
									<div class="form-group">
										...
										<input type="text" id="gds_name" name="gds_name" value='<c:out value="${productVO.gds_name }" />' class="form-control" placeholder="Enter Product name">
										<input type="hidden" id="gds_name" name="gds_name" value='<c:out value="${productVO.gds_num_pk }" />' class="form-control" />
									</div>
									
									<div class="form-group">
										<label for="exampleInputEmail1">제조사</label>
										<input type="text" id="gds_company" name="gds_company" value='<c:out value="${productVO.gds_company }" />' class="form-control" placeholder="Enter company">
									</div>
									...
									<div class="form-group">
										...
										<input style="width:40%; margin-right:10px; display: inline-block;"
											type="text" id="gds_price" name="gds_price" value='<c:out value="${productVO.gds_price }" />' class="form-control" 
											placeholder="Enter price" />
										<input style="width:40%; display: inline-block;"
											type="text" id="gds_discount" name="gds_discount" value='<c:out value="${productVO.gds_discount }" />' class="form-control "
									...
									<div class="form-group">
										...
										<textarea class="form-control" id="gds_detail" name="gds_detail" rows="8"
												placeholder="Enter ..."><c:out value="${productVO.gds_detail }" /></textarea>
									</div>
									
									<div class="form-group">
											<label for="exampleInputEmail1">섬네일 이미지</label>
											<input type="file" id="file1" name="file1" class="form-control" />
											<input type="hidden" id="gds_img" name="gds_img" value='<c:out value="${productVO.gds_img }" />' class="form-control" />
									</div>
									...
									<div class="form-group">
										...
										<input style="width:30%; margin-right:10px; display: inline-block;"
												type="text" id="gds_amount" name='gds_amount' value='<c:out value="${productVO.gds_amount }" />' class="form-control" 
												placeholder="Enter Amount" />
										<select class="form-control" id="gds_buy" name="gds_buy" style="width: 15%; display: inline-block;">
											<option value= "Y" <c:out value="${productVO.gds_buy == 'Y' ? 'selected' : '' }" />>Y</option>
											<option value= "N" <c:out value="${productVO.gds_buy == 'N' ? 'selected' : '' }" />>N</option>
										</select>
